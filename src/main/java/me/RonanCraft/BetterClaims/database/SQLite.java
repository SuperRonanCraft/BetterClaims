...
public class SQLite {
    ...
    public Connection getSQLConnection() {
        if (sqlEnabled) {
            try {
                ...
                e.printStackTrace();
                BetterClaims.getInstance().getLogger().info("MySQL setup is incorrect! Grabbing data from local database!");
                sqlEnabled = false;
            }
        }
        return getLocal();
    ...
    private Connection getLocal() {
        File dataFolder = new File(BetterClaims.getInstance().getDataFolder().getPath() + File.separator + "data", db_file_name + ".db");
        if (!dataFolder.exists()){
            try {
                dataFolder.getParentFile().mkdir();
                dataFolder.createNewFile();
            } catch (IOException e) {
                BetterClaims.getInstance().getLogger().log(Level.SEVERE, "File write error: " + dataFolder.getPath());
                e.printStackTrace();
            }
        }
        try {
            ...
            BetterClaims.getInstance().getLogger().log(Level.SEVERE, "SQLite exception on initialize", ex);
        } catch (ClassNotFoundException ex) {
            BetterClaims.getInstance().getLogger().log(Level.SEVERE, "You need the SQLite JBDC library. Google it. Put it in /lib folder.");
        }
        return null;
    }
    ...
    boolean sqlUpdate(String statement, List<Object> params) {
        ...
        try {
            ...
            BetterClaims.getInstance().getLogger().log(Level.SEVERE, Errors.sqlConnectionExecute(), ex);
            success = false;
        } finally {
            close(ps, null, conn);
        }
    ...
    public void initialize() { //Let in console know if its all setup or not
        ...
        try {
            ...
            BetterClaims.getInstance().getLogger().log(Level.SEVERE, "Unable to retreive connection", ex);
        } finally {
            close(ps, rs, conn);
        }
    }
    ...
    void close(PreparedStatement ps, ResultSet rs, Connection conn) {
        try {
            ...
            Error.close(BetterClaims.getInstance(), ex);
        }
    }
